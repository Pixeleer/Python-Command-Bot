...
from AI import COMMUNICATION,FRAMEWORK as _FRAMEWORK,UpdateData
...
data_keywords = {
    ...
    'enter_pm': ['personal setting','enter personal setting'],
    'leave_pm': ['leave setting','exit setting'],
...
def math(action,context,index,x=None):
    ...
    if action == math_keywords['division_dir']:
        ...
        try:
            ...
            COMMUNICATION.FORMAT.to_error(f'Unable to divide {_one} and {_two}', True)
    ...
    if action == math_keywords['powers_dir']:
        ...
        try:
            ...
            COMMUNICATION.FORMAT.to_error(f'Unable to bring {_one} to the power of {_two}', True)
    ...
    if action == math_keywords['powers_dir_super']:
        ...
        try:
            ...
            COMMUNICATION.FORMAT.to_error(f'Could not bring {_num} to power',True)
            ...
            try:
                ...
                if result != -1:
                    ...
                    if isinstance(Value, dict):
                        ...
                        COMMUNICATION.FORMAT.to_group(f"{x} {nickname}",True)
                    elif isinstance(Value,list):
                        x = COMMUNICATION.FORMAT.to_group(Value, out=False,alone=True)
                        COMMUNICATION.FORMAT.normal(f"{x} {nickname}",True)
                    else:
                        COMMUNICATION.FORMAT.normal(f"{Value} {nickname}",True)
                    return None
            except:
                __excuse__ = None
        COMMUNICATION.FORMAT.normal(f"I don't know {text}", True)
personal,adding = False,False
def process(text,user='Creator'):
    if text.lower() in ['shutdown','shut down']:
        ...
        COMMUNICATION.FORMAT.normal(a[randint(0,len(a)-1)],True)
        return None
    ...
    if context is None:
        COMMUNICATION.FORMAT.to_error(f"There was an error comprehending some of your words i'm afraid", True)
        return
    if personal:
        for index,_ in enumerate(context):
            if _.lower() in data_keywords['leave_pm']:
                personal = False
                COMMUNICATION.FORMAT.normal(f"I'm now assigned to normal settings", True)
                ...
                if Label and Value != "":
                    ...
                    COMMUNICATION.FORMAT.normal(f"Information learned", True)
        if not adding:
    ...
    for index,_ in enumerate(context):
            ...
            if result is not None:
                try:
                    if context[index+2] in _math_keywords:
                        ...
                        won = None
                        COMMUNICATION.FORMAT.to_answer(result, True)
                except:
                    COMMUNICATION.FORMAT.to_answer(result, True)
        ...
        if _.lower() in ['=','equals','equal','is']:
            ...
            if not variable.isnumeric():
                ...
                COMMUNICATION.FORMAT.to_error(f'Cannot assign {variable} to {value}',True)
        ...
        if _.lower() in data_keywords['return_dir']:
            if context[index+1].lower() == 'my':
                ...
                if request.lower() in ['name','username']:
                    COMMUNICATION.FORMAT.normal(f'You are {user}',True)
                elif request.lower() == 'password':
                    COMMUNICATION.FORMAT.to_special(f'That data is locked',True)
                else:
                    ...
                    if result is not None:
                        ...
                        if isinstance(result,(list,tuple,set)):
                            COMMUNICATION.FORMAT.normal(f'Your {c_request} are '
                                                        f'{COMMUNICATION.FORMAT.to_group(result,False,True)}',True)
                        else:
                            COMMUNICATION.FORMAT.normal(f'Your {c_request} is {result}',True)
                    else:
                        COMMUNICATION.FORMAT.to_error(f"Seems I don't have that information. Sorry!",True)

            elif context[index+1].lower() == 'your':
                request = context[index + 2]
                try:
                    info = _FRAMEWORK.DATA.get(f'AI.{request}')
                    COMMUNICATION.FORMAT.normal(info,True)
                except:
                    COMMUNICATION.FORMAT.normal(f'That information is not found in my database',True)
                ...
                try:
                    if context[index+2] not in _math_keywords:
                        try:
                            ...
                            COMMUNICATION.FORMAT.normal(x,True)
                        except:
                            __excuse__ = None
                            # COMMUNICATION.FORMAT.to_error(f'Variable not found',True)
                except:
                    try:
                        request = context[index + 1]
                        x = custom_variables[request]
        ...
        if _.lower() == 'say':
            try:
                COMMUNICATION.FORMAT.normal(" ".join(context[index+1:]), True)
            except:
                __excuse__ = None
        ...
        if _.lower() in data_keywords['enter_pm']:
            if not personal:
                personal = True
                COMMUNICATION.FORMAT.normal(f"I'm now assigned to your personal settings",True)